      ...
      <div className="max-w-screen-md mb-8 lg:mb-16">
          ...
          <div className='flex flex-row gap-4 '>
              <div className='flex flex-col lg:w-1/3 w-full gap-2'>
                  <div className={`menuItems flex justify-between ${(itemNumber===0)?('bg-footerColor text-main'):(' text-active bg-white')} p-5 hover:bg-footerColor hover:text-main`} onClick={()=>{setItemNumber(0)}}>
                      Digital Marketing Services
                      <span onClick={() => setShowMobileMenu(prevState => (prevState === 0 ? 1 : 0))} className='cursor-pointer lg:hidden block select-none'>
                         { (showMobileMenu === 1) ? '-' : '+'}
                      </span>
                  </div>
                  <div className={`itemMenu1 lg:hidden overflow-hidden transition-all duration-700 ease-in-out ${(showMobileMenu === 1) ? 'max-h-full py-10 px-2' : 'max-h-0'}`}>
                  {showMobileMenu!=0 && <ServicesComp itemNumber={showMobileMenu-1} />}
                  </div>
                  <div className={`menuItems flex justify-between ${(itemNumber===1)?('bg-footerColor text-main'):(' text-active bg-white')} p-5 hover:bg-footerColor hover:text-main`} onClick={()=>{setItemNumber(1)}}>
                      Web development
                      <span onClick={() => setShowMobileMenu(prevState => (prevState === 0 ? 2 : 0))} className='cursor-pointer lg:hidden block select-none'>
                      { (showMobileMenu === 2) ? '-' : '+'}
                      </span>
                  </div>
                  <div className={`itemMenu1 lg:hidden overflow-hidden transition-all duration-700 ease-in-out ${(showMobileMenu === 2) ? 'max-h-full py-10 px-2' : 'max-h-0'}`}>
                  {showMobileMenu!=0 && <ServicesComp itemNumber={showMobileMenu-1} />}
                  </div>
                  <div className={`menuItems flex justify-between  ${(itemNumber===2)?('bg-footerColor text-main'):(' text-active bg-white')} p-5 hover:bg-footerColor hover:text-main`} onClick={()=>{setItemNumber(2)}}>
                      Surveillance services
                      <span onClick={() => setShowMobileMenu(prevState => (prevState === 0 ? 3 : 0))} className='cursor-pointer lg:hidden block select-none'>
                      { (showMobileMenu === 3) ? '-' : '+'}
                      </span>
                  </div>
                  <div className={`itemMenu1 lg:hidden overflow-hidden transition-all duration-700 ease-in-out ${(showMobileMenu === 3) ? 'max-h-full py-10 px-2' : 'max-h-0'}`}>
                    {showMobileMenu!=0 && <ServicesComp itemNumber={showMobileMenu-1}  />}
                    </div>
