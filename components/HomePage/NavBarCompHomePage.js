...
function NavbarCompHomePage(props) {
  ...
  const [textColor, setTextColor] = useState(
    props.textColor ? props.textColor : "main"
  );
  const [isHovered, setIsHovered] = useState(false);
  const toggleHover = () => {
    setIsHovered(!isHovered);
    if (theme === "dark" && props.theme === "light") setTheme("light");
    else setTheme("dark");

    if (textColor === "main" && props.textColor === "gray-900")
      setTextColor("gray-900");
    else setTextColor("main");
  };

  return (
    <>
      <div
        className={`w-full relative z-40 hidden sm:grid lg:grid-cols-12 sm:grid-cols-7 ${
          className ? className : ""
        }`}
      >
        {theme === "dark" && (
